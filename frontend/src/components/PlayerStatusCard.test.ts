...
describe('PlayerStatusCard', () => {
  it('renders identity, turn classes, metrics and missing values with the existing text', () => {
    ...
    expect(card.findAll('.metric-badge').map(item => item.text())).toEqual(['2', '7最高 5', '4'])
    ...
    expect(opponent.findAll('.metric-badge').map(item => item.text())).toEqual(['0', '0最高 0', '0'])
  })
  ...
  it('keeps two fixed token rows, overflow rows, and development-card color/order stacking', () => {
    ...
    expect(rows.flatMap(row => row.findAll('.token-cell')).every(cell => Boolean(cell.attributes('aria-label')))).toBe(true)
  ...
  it('shows noble images in source order only while hovered and forwards image errors', async () => {
    ...
    expect(images.map(image => image.attributes('alt'))).toEqual(['2分+新回合', '贵族unknown'])
