...
describe('PlayerStatusCard', () => {
  ...
  it('keeps two fixed token rows, overflow rows, and development-card color/order stacking', () => {
    ...
    expect(rows[0].findAll('img').map(image => image.attributes('alt'))).toEqual(['white', 'white', 'white', 'white', 'white'])
    expect(rows[1].findAll('img').map(image => image.attributes('alt'))).toEqual(['white', 'blue', 'blue', 'blue', 'blue'])
    expect(rows[2].findAll('img').map(image => image.attributes('alt'))).toEqual(['blue', 'blue'])
